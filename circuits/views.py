...
def _shop_payloads() -> list[dict]:
    return [
        ShopResponse.model_validate(shop).model_dump()
        for shop in _service.list_shops()
    ]
...
def shop_list_and_create_view(request: HttpRequest) -> HttpResponse:
    if request.method == "GET":
        ...
        shops = _service.list_shops(neighborhood)
        payload = [ShopResponse.model_validate(shop).model_dump() for shop in shops]
...
def circuit_list_and_create_view(request: HttpRequest) -> HttpResponse:
    if request.method == "GET":
        payload = [
            _circuit_to_payload(circuit) for circuit in _service.list_circuits()
        ]
        return JsonResponse({"results": payload})
...
def _circuit_to_payload(circuit) -> dict:
    ordered_stops = list(
        CircuitStop.objects.filter(circuit=circuit).select_related("shop").order_by("position").all()
    )
